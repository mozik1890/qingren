var Qixi=function(){
	var confi={
		...
		audio:{
			enable:true,
			playURl:"http://www.170mv.com/kw/other.web.rg01.sycdn.kuwo.cn/resource/n1/39/4/3057850546.mp3",
	...
	var debug=0;
	if(debug){
		$.each(confi.setTime,function(key,val){confi.setTime[key]=500})
	}
	...
	var animationEnd=(function(){
		...
		if(~explorer.indexOf("WebKit")){return"webkitAnimationEnd"}
		return"animationend"})();
	if(confi.audio.enable){var audio1=Hmlt5Audio(confi.audio.playURl);
	audio1.end(function(){Hmlt5Audio(confi.audio.cycleURL,true)})
}
var swipe=Swipe(container);
function scrollTo(time,proportionX){var distX=visualWidth*proportionX;swipe.scrollTo(distX,time)}var girl={elem:$(".girl"),getHeight:function(){return this.elem.height()},rotate:function(){this.elem.addClass("girl-rotate")
...
function Swipe(container,options){
	...
	swipe.scrollTo=function(x,speed){
		element.css({"transition-timing-function":"linear","transition-duration":speed+"ms","transform":"translate3d(-"+x+"px,0px,0px)"});return this};return swipe};
